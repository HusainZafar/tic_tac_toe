...
class tic_tac_toe:
	...
	def minimax(self, board, move, comp, plr):
		"""
		...
		for i in range(9):
			if board[i] == '-':
				if move == comp:
					...
					next_move = comp
				board[i] = move
				minimax_val = self.minimax(board, next_move, comp, plr)
				board[i] = '-'
		...
		if order == 1:
			...
			while utils.check_win(board, comp, plr)[0] == 0:
				...
				if utils.check_empty(board):
					...
					tut = [-i for i in self.minimax(board, plr, comp, plr)]
				if t == 1:
					utils.clearScreen()
				...
				if utils.check_win(board, comp, plr)[0] != 0:
					break
				ret = self.minimax(board, comp, comp, plr)
